...
**These constraints are scoped to the narrative journey (`index.html`), not to the resident pages.** The directory is a different kind of object and needs the opposite affordances: a few hundred resources have to be scannable, so they are rows in a grid, each bucket carries a heading, each page carries a rail of what is on it, and every surface is labeled. The resident chapter on the home page still obeys the rule — it is a flowing list of sentences, not a mosaic of tiles.
